...
- **Rows are capped at the server.** `vacuum.console.max_rows` is applied by wrapping your statement in a subquery with a `LIMIT`, so the rows beyond the cap are never produced. This bounds the number of rows and **not the width of one**: `SELECT repeat('x', 500000000)` is a single row, and is bounded only by `statement_timeout` and PHP's `memory_limit`.
