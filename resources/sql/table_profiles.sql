...
--
-- This is table_profile.sql with the predicate that names one table replaced by the
-- one every whole-connection query already carries, kept as its own file rather than
-- a parameterised one, because every table largest first is a different question
-- from one table by name, and a statement with an optional predicate is harder to
-- read than two statements that each say what they mean.
--
-- The ignored schemas are honoured here for the same reason IndexStatistics honours
-- them: a reader who has told Vacuum not to report on a schema has not made an
-- exception for the pages that sweep every table.
SELECT
    stats.schemaname,
    stats.relname,
...
WHERE stats.schemaname <> ALL (string_to_array(?, ','))
ORDER BY pg_total_relation_size(tables.oid) DESC
